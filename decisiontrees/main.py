...
def get_best_tree(data, criterion, name="", graph=False):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        source_vars, target_vars, test_size=0.6, random_state=0)
    ...
    if graph:
        ...
        plot_name = "images/plot0_" + criterion + \
            ".png" if name == "" else "images/" + name + "_plot0_" + criterion + ".png"
        ...
        plot_name = "images/plot1_" + criterion + \
            ".png" if name == "" else "images/" + name + "_plot1_" + criterion + ".png"
        plt.savefig(plot_name)
...
def visualize_tree(tree, feature_names, class_names, criterion, name=""):
    ...
    plot_name = "images/plot2_" + criterion + \
        ".png" if name == "" else "images/" + name + "_plot2_" + criterion + ".png"
...
def main():
    ...
    entropy_tree, entropy_train_scores, entropy_test_scores, entropy_node_counts = get_best_tree(
        data, criterion="entropy", graph=graph, name=out_name)
    gini_tree, gini_train_scores, gini_test_scores, gini_node_count = get_best_tree(
        data, criterion="gini", graph=graph, name=out_name)
    ...
    if graph:
        ...
        visualize_tree(entropy_tree, feature_names,
                       class_names, "entropy", name=out_name)
        visualize_tree(gini_tree, feature_names,
